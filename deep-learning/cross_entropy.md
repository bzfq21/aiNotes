...
## 2. 交叉熵 (Cross-Entropy) - 从编码角度理解

### 2.1 编码问题的直观理解

**核心问题**：如果我们用基于分布 $Q$ 的最优编码方案来对来自真实分布 $P$ 的数据进行编码，平均需要多少比特？

**举例**：
- 英语中字母"e"出现频率高，"z"出现频率低
- 如果我们用固定长度编码所有字母，效率不高
- 最优编码应该给高频字母短编码，低频字母长编码
...
**关键分解**：
$$H(P, Q) = \underbrace{-\sum P(x) \log P(x)}_{H(P):\text{真实熵}} + \underbrace{\sum P(x) \log\frac{P(x)}{Q(x)}}_{\text{额外比特数}}$$

**理解**：
- 第一项 $H(P)$：知道真实分布 $P$ 时的最小编码长度
- 第二项：由于使用错误的分布 $Q$ 导致的额外编码长度

### 2.4 机器学习中的应用

在分类问题中：
- **真实分布**：$P(y|x)$ 通常是 one-hot 编码（如 $[0,0,1,0]$）
- **预测分布**：$Q(y|x)$ 是模型的 softmax 输出（如 $[0.1,0.2,0.6,0.1]$）
...
**编码解释**：
- $H(P, Q)$：用基于 $Q$ 的编码方案对来自 $P$ 的数据编码的平均比特数
- $H(P)$：用基于 $P$ 的编码方案对来自 $P$ 的数据编码的最小平均比特数
- $D_{KL}(P \| Q)$：由于使用错误的分布 $Q$ 导致的**额外编码长度**

**信息损失**：KL散度衡量了当我们用分布 $Q$ 来近似真实分布 $P$ 时，丢失了多少信息。
...
**不满足三角不等式**：KL散度不是真正的距离度量，而是**相对熵**或**信息增益**

### 3.4 KL散度的信息论解释

**假设检验角度**：
- 当我们需要区分分布 $P$ 和 $Q$ 时
- KL散度衡量了区分这两个分布所需的**对数似然比期望值**

**压缩角度**：
- 使用基于 $Q$ 的编码方案对来自 $P$ 的数据进行编码时
- 每个符号额外的平均比特数就是KL散度
